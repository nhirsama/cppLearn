...
void nhir() {
    i32 n;
    std::cin >> n;
    std::vector<Point<long double>> point(n);
    for (auto &i: point) {
        std::cin >> i;
    }
    long double ans = 0;
    for (i32 i = 1; i < n; i++) {
        long double e = distance(point[i], point[i - 1]);
        long double k = std::max(0.0, log2(e * log(2.0)));
//        printf("%.10Lf\n", k);

//        ans += 2.0 * k + 2.0 * e / pow(2.0, k);
//        long double l = 0, r = 100;
//        while (l + eps < r) {
//            long double mid = (l + r) / 2.0;
//            long double de = 2.0 - 2.0 * e * log(2.0) * pow(2.0, -mid);
//            if (de > 0) r = mid;
//            else l = mid;
//        }
//        k = (l + r) / 2.0;
