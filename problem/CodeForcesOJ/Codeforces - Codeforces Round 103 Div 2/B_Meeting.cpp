...
#define cc(x) cout << fixed << setprecision(x);
    ...
    template<class T>
    Point<T> rotate(Point<T> p1, Point<T> p2) {
        // 旋转
        Point<T> vec = p1 - p2;
        return {-vec.y, vec.x};
    }
...
void nhir() {
    ...
    i32 n;
    std::cin >> n;
    std::vector<std::pair<Geometry::Point<i32>, i32> > v(n);
    for (auto &[x,y]: v) {
        std::cin >> x >> y;
    }
    std::set<Geometry::Point<i32> > p;
    for (i32 i = std::min(xa, xb); i <= std::max(xa, xb); i++) {
        p.insert(Geometry::Point<i32>(i, ya));
        p.insert(Geometry::Point<i32>(i, yb));
    }
    for (i32 i = std::min(ya, yb); i <= std::max(ya, yb); i++) {
        p.insert(Geometry::Point<i32>(xa, i));
        p.insert(Geometry::Point<i32>(xb, i));
    }
    for (auto &[pp,r]: v) {
        std::vector<Geometry::Point<i32> > erase;
        for (auto x: p) {
            if (dis(x, pp) <= r) {
                erase.push_back(x);
            }
