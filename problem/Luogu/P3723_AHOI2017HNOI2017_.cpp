...
void nhir() {
    ...
    int x2 = x + 1;
//    std::cout << x << endl;
    ans += std::min(n * x * x + 2 * x * cf, std::min(n * x1 * x1 + 2 * x1 * cf, n * x2 * x2 + 2 * x2 * cf));
    std::vector<int> ta(2 * n);
    for (int i = 0; i < n; i++) {
        ta[2 * n - i - 1] = ta[n - i - 1] = a[i];
    }
    Poly pta(ta), pb(b);
    pta *= pb;
    int max = 0;
    for (int i = n; i < 2 * n; i++) {
        max = std::max(max, pta.a[i]);
    }
    std::cout << ans - 2 * max << endl;
}
