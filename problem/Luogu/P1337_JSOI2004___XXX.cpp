...
void nhir() {
    Geometry::Point<f128> ans;
    f128 ansm = 0;
    int n;
    std::cin >> n;
    std::vector<Geometry::Point<long double>> point(n);
    std::vector<int64> m(n);
    for (int i = 0; i < n; i++) {
        std::cin >> point[i] >> m[i];
        ans += point[i] * m[i];
        ansm += m[i];
    ...
    ans /= ansm;
    auto energy = [&](Geometry::Point<f128> npo) {
        f128 now = 0;
        for (int i = 0; i < n; i++) {
            now += Geometry::dis(point[i], npo) * m[i];
        }
        return now;
    };
    ansm = energy(ans);
    ...
    std::uniform_real_distribution<f128> ran(0.0, 1.0);
    auto th = [&]() {
        f128 t = 3000;
        while (t > 1e-15) {
//            for (int i = 0; i < 50; i++) {
            Geometry::Point<f128> newp((rand() * 2 - RAND_MAX) * t, (rand() * 2 - RAND_MAX) * t);

//                Geometry::Point<f128> newp((ran(random) * 2 - 1) * t, (ran(random) * 2 - 1) * t);
            newp += ans;
            f128 ene = energy(newp);
            f128 d = ene - ansm;
            if (d < 0 || std::exp(-d / t) * RAND_MAX > rand()) {
                ansm = ene;
                ans = newp;
            }
//            }
    ...
    th();
    th();
    std::cout << std::format("{:.3f} {:.3f}", ans.x, ans.y) << endl;
}
...
signed main() {
    ...
    std::cin.tie(nullptr);
    int T = 1;
    if (OJ_NAME != "AtCoder" && OJ_NAME != "Luogu")
        std::cin >> T;
    while (T--) nhir();
    return 0;
}